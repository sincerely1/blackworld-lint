 ...
 * @param cwd
 * @param pkg
 * @returns blackworld-eslint-config/index
 * @returns blackworld-eslint-config/react
 * @returns blackworld-eslint-config/typescript/index
 * @returns blackworld-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'blackworld-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
